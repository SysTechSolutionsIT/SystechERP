...
const CurrencyMaster = require("../model/CurrenciesModel"); // Import your CurrencyMaster model
...
router.get("/", authToken, async (req, res) => {
  const records = await CurrencyMaster.findAll(); // Retrieve all bank records
...
router.get("/:id", authToken, async (req, res) => {
  const record = await CurrencyMaster.findByPk(req.params.id);
...
router.post("/add-currency", authToken, async (req, res) => {
  try {
    const newRecord = await CurrencyMaster.create(req.body); // Create a new bank record based on the request body
...
router.patch("/update-record/:id", authToken, async (req, res) => {
  ...
  try {
    const updatedRecord = await CurrencyMaster.findByPk(DeviceId);
...
router.delete("/delete-record/:id", authToken, async (req, res) => {
  ...
  try {
    const deleteRecord = await CurrencyMaster.findByPk(bankId);
