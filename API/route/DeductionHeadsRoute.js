...
router.get("/get/:id", async (req, res) => {
  try {
    const DeductionHeadByID = await DeductionHeadsMaster.findByPk(
      req.params.id
    );
...
router.patch("/update/:id", async (req, res) => {
  ...
  try {
    const updatedDeductionHeads = await DeductionHeadsMaster.findByPk(
      DeductionHeadId
    );
...
router.delete("/delete/:id", async (req, res) => {
  const DeductionHeadId = req.params.id; // Get the deduction head record ID from the URL parameter
  try {
    const deletedDeductionHeads = await DeductionHeadsMaster.findByPk(
      DeductionHeadId
    );
