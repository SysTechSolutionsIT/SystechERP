...
const EarningImportModal = ({ visible, onClick, edit }) => {
    const { token } = useAuth();
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, {resetForm}) => {
      ...
      addMonthlyEarningImport(updatedData)
      resetForm()
      onClick()
    }
  });
  ...
  const addMonthlyEarningImport = async(data) =>{
    try {
        const response = await axios.post('http://localhost:5500/monthly-earning-import/FnAddUpdateDeleteRecord',
        data,
        {
            headers: { Authorization: `Bearer ${token}`}
        })
        alert('Monthly Earning Import Added')
    } catch (error) {
        console.error('Error', error);
    }
  }
      ...
      <div className="fixed overflow-y-scroll inset-0 bg-black bg-opacity-25 backdrop-blur-sm flex items-center justify-center w-full">
        <div className="bg-gray-200 w-[60%] overflow-y-scroll max-h-fit p-8 rounded-lg">
          ...
          <div className='mt-1'>
                ...
                >
                  <option value="">Select Earning Head to Import</option>
...
export default EarningImportModal;
